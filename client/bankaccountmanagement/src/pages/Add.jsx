...
function Add() {
    const [formData,setFormData]=useState({
        name:'',place:'',email:'',phone:'',account_no:''
    })
    ...
    const getData=(e)=>{
        ...
        const {name,place,email,phone,account_no}=formData
        if (!name || !place || !email || !phone || !account_no){
            toast.warning('Invalid input')
        }
        else{
            addBooking({name,place,email,phone,account_no}).then((res)=>{
                console.log(res.data);
                toast.success(' Added Successfully')
                setFormData({name:'',place:'',email:'',phone:'',account_no:''})
                navigate('/')
            }).catch((err)=>{
                console.log(err);
            ...
            <div className="row d-flex align-items-center justify-content-center">
                ...
                <div className='col-md-7'>
                    <form action="">
                        ...
                        <FloatingLabel controlId="floatingSalary" label="Enter your 12-digits account number" className="mb-3">
                            <Form.Control type="number" placeholder="account_no" onChange={(e)=>setFormData({...formData,account_no:e.target.value})}/>
                        </FloatingLabel>
